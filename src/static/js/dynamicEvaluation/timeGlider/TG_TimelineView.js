    ...
    tg.TG_TimelinePlayer = function (widget, mediator) {
        ...
        */
		...
		+ "</div>"),
          ...
          "<div class='tg-legend-controls'><span class='tg-legend-all'>显示所有</span><span class='tg-legend-none'> | 隐藏所有</span></div>" +
          "</div>" +
          "</div>")
    ...
    tg.TG_TimelinePlayer.prototype = {
        ...
        */
        ...
        setupSingleTimeline: function () {
			...
			leg = (timeline.get("hasLegend")) ? "<li id='legend' class='tg-legend-bt' data-timeline_id='" + tid + "'>类型</li>" : "",

			tools = ""; // "<a id='tools' class='tools-bt noselect'>tools</a>",

                tmpl = "<div class='tg-widget-header tg-single-timeline-header'>" + title + "<ul>" + inf + leg + "<li class='tg-timeline-start' data-timeline_id='" + tid + "'></li></ul>" + tools + "</div>";
        ...
        */
        createEventLinksMenu: function (linkage, objectId, objectType) {
            if (!linkage) return "";
            console.log(linkage);
            var html = '', l = 0, lUrl = "", lLab = "";
            console.log(typeof (linkage));
            if (typeof (linkage) == "string") {
                ///single url string for link: use "link"
                html = "<li><a href='" + linkage + '?' + objectId + '&' + objectType + "' target='_blank'>表2</a></li>"
            } else if (typeof (linkage) == "object") {
                // array of links with labels and urls
                for (l = 0; l < linkage.length; l++) {
                    lUrl = linkage[l].url;
                    lLab = linkage[l].label;
                    html += "<li><a href='" + lUrl + '?' + objectId + '&' + objectType + "' target='_blank'>" + lLab + "</a></li>"
                }
            }
            return html;
        },
        ...
        eventModal: function (eid, $event) {
			...
			links = this.createEventLinksMenu(ev.link, ev.objectId, ev.objectType),
