Sidebar.KLG = function (editor) {
    ...
    container.addStatic(new UI.Text('克里格储量估算'));
    ...
    function updateUI() {
        ...
        if (object !== null && editor.getObjectType(object) === 'object' && !object.ori) {
            ...
            var jvlimi = new UI.Button('储量估算').onClick(function () {
                ...
                post_data.k_type = klgtype_pl.getValue();
                post_data.it_type = bymodetype_pl.getValue();
                post_data.f_co = kuaijin.getValue();
                post_data.f_cc = jitai.getValue();
                post_data.fk_mean = fk.getValue();
                ...
                $.ajax({
                    ...
                    url: "../getdata/WellInfo2.asmx/KLGCompute", //调用WebService的地址和方法名称组合 ---- WsURL/方法名
            ...
            var klg_options = { "0": "简单克里格", "1": "普通克里格", "2": "对数正态克里格" };
            var klgtype_panel = new UI.Panel();
            var klgtype_pl = new UI.Select().setOptions(klg_options).setWidth('150px').setColor('#444').setFontSize('12px');
            klgtype_panel.add(new UI.Text('克里格类型').setWidth('90px'));
            klgtype_panel.add(klgtype_pl);
            params.add(klgtype_panel);

            var bymode_options = { "1": "球型模型", "2": "指数模型", "3": "高斯模型" };
            var bymodetype_panel = new UI.Panel();
            var bymodetype_pl = new UI.Select().setOptions(bymode_options).setWidth('150px').setColor('#444').setFontSize('12px');
            bymodetype_panel.add(new UI.Text('变异模型类型').setWidth('90px'));
            bymodetype_panel.add(bymodetype_pl);
            params.add(bymodetype_panel);

            var kuaijin_panel = new UI.Panel();
            var kuaijin = new UI.Number().setWidth('150px');
            kuaijin_panel.add(new UI.Text('块金值：').setWidth('90px'));
            kuaijin.setValue(0.02);
            kuaijin_panel.add(kuaijin);
            params.add(kuaijin_panel);

            var jitai_panel = new UI.Panel();
            var jitai = new UI.Number().setWidth('150px');
            jitai_panel.add(new UI.Text('基台减块金：').setWidth('90px'));
            jitai.setValue(0.16249);
            jitai_panel.add(jitai);
            params.add(jitai_panel)

            var fk_panel = new UI.Panel();
            var fk = new UI.Number().setWidth('150px');
            fk_panel.add(new UI.Text('fkmean：').setWidth('90px'));
            fk.setValue(0.0);
            fk_panel.add(fk);
            params.add(fk_panel);
